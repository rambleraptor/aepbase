...
func RegisterRoutes(mux *http.ServeMux, state StateManager) {
	mux.HandleFunc("POST /definitions", makeCreateHandler(state))
	mux.HandleFunc("GET /definitions", makeListHandler(state))
	mux.HandleFunc("GET /definitions/{definition}", makeGetHandler(state))
	mux.HandleFunc("PATCH /definitions/{definition}", makeUpdateHandler(state))
	mux.HandleFunc("DELETE /definitions/{definition}", makeDeleteHandler(state))
}
...
func insertDefinition(db *sql.DB, def *ResourceDefinition) error {
	...
	_, err := db.Exec(
		"INSERT INTO _definitions (id, singular, plural, description, examples_json, schema_json, parents_json, create_time, update_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
func getDefinitionByID(db *sql.DB, id string) (*ResourceDefinition, error) {
	row := db.QueryRow("SELECT id, singular, plural, description, examples_json, schema_json, parents_json, create_time, update_time FROM _definitions WHERE id = ?", id)
	return scanDefinition(row)
}

func getDefinition(db *sql.DB, singular string) (*ResourceDefinition, error) {
	row := db.QueryRow("SELECT id, singular, plural, description, examples_json, schema_json, parents_json, create_time, update_time FROM _definitions WHERE singular = ?", singular)
	return scanDefinition(row)
}
...
func listDefinitions(db *sql.DB, limit int, cursor string) ([]ResourceDefinition, error) {
	...
	if cursor != "" {
		rows, err = db.Query(
			"SELECT id, singular, plural, description, examples_json, schema_json, parents_json, create_time, update_time FROM _definitions WHERE id > ? ORDER BY id LIMIT ?",
			cursor, limit,
		)
	} else {
		rows, err = db.Query(
			"SELECT id, singular, plural, description, examples_json, schema_json, parents_json, create_time, update_time FROM _definitions ORDER BY id LIMIT ?",
			limit,
		)
	}
	if err != nil {
...
func updateDefinition(db *sql.DB, def *ResourceDefinition) error {
	...
	examplesJSON, _ := json.Marshal(def.Examples)
	_, err := db.Exec(
		"UPDATE _definitions SET description = ?, examples_json = ?, schema_json = ?, update_time = ? WHERE id = ?",
...
func deleteDefinition(db *sql.DB, id string) error {
	_, err := db.Exec("DELETE FROM _definitions WHERE id = ?", id)
	return err
}
