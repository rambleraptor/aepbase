...
func TestCreateResource(t *testing.T) {
	...
	if m["path"] != "definitions/publisher" {
...
func TestCreateResourceDuplicate(t *testing.T) {
	...
	resp := doRequest(t, h, "POST", "/definitions?id=publisher", body)
...
func TestCreateResourceMissingFields(t *testing.T) {
	state := newTestState(t)
	h := state.Handler()
	resp := doRequest(t, h, "POST", "/definitions", `{"singular":"","plural":""}`)
...
func TestCreateResourceInvalidParent(t *testing.T) {
	...
	resp := doRequest(t, h, "POST", "/definitions?id=book", body)
	if resp.StatusCode != 400 {
		t.Errorf("expected 400, got %d", resp.StatusCode)
	}
}
...
func TestGetResource(t *testing.T) {
	...
	resp := doRequest(t, h, "GET", "/definitions/publisher", "")
	if resp.StatusCode != 200 {
...
func TestGetResourceNotFound(t *testing.T) {
	state := newTestState(t)
	h := state.Handler()
	resp := doRequest(t, h, "GET", "/definitions/nonexistent", "")
...
func TestListResources(t *testing.T) {
	...
	resp := doRequest(t, h, "GET", "/definitions", "")
	if resp.StatusCode != 200 {
		t.Fatalf("expected 200, got %d", resp.StatusCode)
	}
	m := readJSON(t, resp)
...
func TestDeleteResource(t *testing.T) {
	...
	resp := doRequest(t, h, "DELETE", "/definitions/publisher", "")
...
func TestUpdateResourceAddColumn(t *testing.T) {
	...
	resp := doRequest(t, h, "PATCH", "/definitions/publisher", patch)
	if resp.StatusCode != 200 {
		m := readJSON(t, resp)
...
func TestUpdateResourceChangeParents(t *testing.T) {
	...
	resp := doRequest(t, h, "PATCH", "/definitions/book", patch)
	if resp.StatusCode != 400 {
...
func TestOpenAPIAfterDelete(t *testing.T) {
	...
	doRequest(t, h, "DELETE", "/definitions/publisher", "")
	...
	for p := range paths {
		if p != "/definitions" && p != "/definitions/{definition_id}" {
...
func TestCreateRejectsWhenRequiredFieldMissing(t *testing.T) {
	...
	b, _ := json.Marshal(body)
	resp := doRequest(t, h, "POST", "/definitions?id=publisher", string(b))
	if resp.StatusCode != 200 {
		m := readJSON(t, resp)
...
func TestCreateSucceedsWhenRequiredFieldPresent(t *testing.T) {
	...
	b, _ := json.Marshal(body)
	doRequest(t, h, "POST", "/definitions?id=publisher", string(b))
...
func TestOpenAPIIncludesRequiredFieldsAfterRestart(t *testing.T) {
	...
	b, _ := json.Marshal(body)
	resp := doRequest(t, h1, "POST", "/definitions?id=publisher", string(b))
	if resp.StatusCode != 200 {
		t.Fatalf("createResource: status %d", resp.StatusCode)
	}
	d1.Close()
...
func TestDeleteResourceWithGrandchildren(t *testing.T) {
	...
	resp := doRequest(t, h, "DELETE", "/definitions/book", "")
	if resp.StatusCode != 400 {
		t.Errorf("expected 400 (has children), got %d", resp.StatusCode)
	}

	// Delete chapter first, then book, then publisher.
	resp = doRequest(t, h, "DELETE", "/definitions/chapter", "")
	if resp.StatusCode != 204 {
